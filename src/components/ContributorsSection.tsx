              ...
              <div>
                ...
                <div className="flex items-start justify-between gap-4 mb-5">
                  ...
                  <span className="font-sans text-[10px] font-semibold text-paper/50 bg-paper/5 border border-paper/10 px-2.5 py-1 rounded-sm shrink-0 flex items-center gap-1">
                    ...
                    {authorArticles.length} {authorArticles.length === 1 ? 'Paper' : 'Papers'}
                  </span>
                </div>

                {/* Short Bio */}
                <p className="font-serif text-sm text-paper/70 leading-relaxed line-clamp-3 mb-6">
                  {profile.bio}
                </p>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 pb-6 border-b border-paper/10 pr-8">
                {activeContributor.avatarUrl ? (
                  <img 
                    src={activeContributor.avatarUrl} 
              ...
              <div className="pt-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-sans text-[11px] font-bold tracking-[0.2em] uppercase text-paper flex items-center gap-2">
                    ...
                    Published Research Papers ({getContributorArticles(activeContributor).length})
                  </h3>
                </div>

                {getContributorArticles(activeContributor).length > 0 ? (
                  <div className="space-y-4">
                    {getContributorArticles(activeContributor).map(art => (
                      <div 
                        key={art.id}
